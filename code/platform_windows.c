...
static void allTests_(Arena* arena, Platform* platform) {tempMemoryBlock(arena) {
...

    {
        ...
        assert(cmdData.keybindings.len == InputKey_Count);
        clearArgs(&cmdData);
        addArg(&cmdData, STR("cmdkeybind"));
        addArg(&cmdData, STR("a"));
        addArg(&cmdData, STR("cmdlist"));
        cmdkeybind(&cmdData);
        assert(streq(currentEntry(&iter)->str, STR("Bound \"a\" to \"cmdlist\"")));
        addArg(&cmdData, STR("cmdexec"));
        cmdkeybind(&cmdData);
        assert(streq(nextEntry(&iter)->str, STR("Bound \"a\" to \"cmdlist cmdexec\"")));
        assert(cmdData.keybindings.ptr[InputKey_a].arena.used == sizeof("cmdlist cmdexec"));

        clearArgs(&cmdData);
        addArg(&cmdData, STR("cmdkeybind"));
        cmdkeybind(&cmdData);
        assert(streq(nextEntry(&iter)->str, STR("usage: keybind <key> <command(s)>")));
        ...
        cmdkeybind(&cmdData);
        assert(streq(nextEntry(&iter)->str, STR("\"b\" is not currently bound")));
